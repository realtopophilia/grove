...
export function getRcoEvents() {
  try {
    return load('rco-events.json')
  } catch {
    return []
  }
}
...
export function getNeighborhoodBySlug(slug) {
  ...
  const n = all.find(n => n.slug === slug)
  if (!n) return null

  // Attach upcoming events for orgs in this neighborhood
  const events = getRcoEvents()
  const orgIds = new Set([
    ...n.rcos.map(r => r.id),
    ...n.orgs.map(o => o.id),
  ])
  n.events = events.filter(e => orgIds.has(e.orgId))
