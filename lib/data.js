...
export function getEventbriteEvents() {
  return load('eventbrite-events.json')
}

export function getMeetupEvents() {
  return load('meetup-events.json')
}
...
export function getNeighborhoodIndex() {
  ...
  const ebEvents = getEventbriteEvents()
  const meetupEvents = getMeetupEvents()
  const rcos = getRcos()
  ...
  const ensure = (name) => {
    if (!map[name]) map[name] = { name, slug: slugify(name), clpEvents: [], parksEvents: [], ebEvents: [], meetupEvents: [], rcos: [], orgs: [] }
  }
  ...
  for (const ev of ebEvents) {
    if (ev.neighborhood) {
      ...
      map[ev.neighborhood].ebEvents.push(ev)
    }
  }
  ...
  for (const ev of meetupEvents) {
    if (ev.neighborhood) {
      ...
      map[ev.neighborhood].meetupEvents.push(ev)
    }
  }
